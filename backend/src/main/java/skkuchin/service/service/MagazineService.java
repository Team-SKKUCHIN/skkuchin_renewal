...
public class MagazineService {
    ...
    public void addMagazine(AppUser appUser, MagazineDto.PostRequest dto) {
        ...
        while (!placeIds.isEmpty()) {
            ...
            Place place = placeRepo.findById(placeId)
                    .orElseThrow(() -> new CustomValidationApiException("존재하지 않는 식당 아이디 입니다."));
    ...
    public MagazineDto.Response getSpecificMagazine(Long magazineId) {
        Magazine magazine = magazineRepo.findById(magazineId)
                .orElseThrow(() -> new CustomValidationApiException("존재하지 않는 매거진입니다."));
    ...
    public void deleteMagazine(AppUser appUser, Long magazineId) {
        Magazine magazine = magazineRepo.findById(magazineId)
                .orElseThrow(() -> new CustomValidationApiException("존재하지 않는 매거진 입니다."));
    ...
    public void updateMagazine(AppUser appUser, Long magazineId, MagazineDto.PutRequest dto) {
        Magazine beforeMagazine = magazineRepo.findById(magazineId)
                .orElseThrow(() -> new CustomValidationApiException("존재하지 않는 매거진 입니다."));
        ...
        while (!placeIds.isEmpty()) {
            Long placeId = placeIds.remove(0);
            Place place = placeRepo.findById(placeId)
                    .orElseThrow(() -> new CustomValidationApiException("존재하지 않는 식당 Id 입니다."));
    ...
    private void canHandleMagazine(AppUser magazineUser, AppUser user) {
        if (!(magazineUser.getId().equals(user.getId())
